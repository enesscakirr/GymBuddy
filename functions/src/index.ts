...
export const analyzeFood = functions
  ...
  .https.onCall(async (data: { imageBase64: string; mimeType?: string }) => {
    try {
      const apiKey = process.env.GEMINI_API_KEY;
      console.log("API key mevcut:", !!apiKey);
      if (!apiKey) throw new functions.https.HttpsError("internal", "API anahtarı bulunamadı.");
      ...
      if (!jsonMatch) throw new functions.https.HttpsError("internal", "AI yanıtı JSON içermiyor.");

      const parsed = JSON.parse(jsonMatch[0]);
      if ("error" in parsed) return { error: parsed.error };

      // Her besin için detaylı veri
      const foods: Array<{
        name: string; grams: number;
        calories: number; protein: number; carbs: number; fat: number;
        fiber: number; sugar: number; saturatedFat: number;
        sodium: number; cholesterol: number; potassium: number;
      }> = [];
      ...
      return {
        ...
        servingSize: `${foods[0]?.grams || 100}g`,
